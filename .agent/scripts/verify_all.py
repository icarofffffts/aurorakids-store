...
def run_script(name: str, script_path: Path, project_path: str, url: Optional[str] = None, required: bool = False) -> dict:
    ...
    if not script_path.exists():
        ...
        return {"name": name, "passed": True, "skipped": True, "duration": 0, "required": required}
    ...
    try:
        ...
        return {"name": name, "passed": False, "skipped": False, "duration": duration, "error": "Timeout", "required": required}
    
    except Exception as e:
        duration = (datetime.now() - start_time).total_seconds()
        print_error(f"{name}: ERROR - {str(e)}")
        return {"name": name, "passed": False, "skipped": False, "duration": duration, "error": str(e), "required": required}
...
def print_final_report(results: List[dict], start_time: datetime):
    ...
    failed_required = sum(1 for r in results if (not r["passed"]) and (not r.get("skipped")) and r.get("required"))
    ...
    # Final verdict: only REQUIRED failures block deployment.
    if failed_required > 0:
        print_error(f"VERIFICATION FAILED - {failed_required} required check(s) failed")
        print(f"\n{Colors.YELLOW}💡 Tip: Fix required checks first (security, lint, perf){Colors.ENDC}")
        return False

    if failed > 0:
        print_warning(f"OPTIONAL CHECKS FAILED - {failed} check(s) to improve")
        print_success("✨ REQUIRED CHECKS PASSED - OK to deploy (with improvements pending) ✨")
        return True
  ...
  python scripts/verify_all.py . --url https://staging.example.com --no-e2e
    ...
    for suite in VERIFICATION_SUITE:
        ...
        for name, script_path, required in suite["checks"]:
            ...
            # Stop on REQUIRED failure if flag set
