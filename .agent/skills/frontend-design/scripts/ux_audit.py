...
class UXAuditor:
    ...
    def audit_file(self, filepath: str) -> None:
        ...
        # Avoid CSS false-positives (e.g. "input { ... }")
        if ext == '.css':
            has_form = False
        else:
            # Avoid false positives from plain words like "card" or "payment".
            has_form = bool(
                re.search(r'<form\b|<input\b|<select\b|<textarea\b', content, re.IGNORECASE)
                or re.search(r'type=["\"]password|type=[\'\"]password', content, re.IGNORECASE)
            )
        complex_elements = len(re.findall(r'<input|<select|<textarea|<option', content, re.IGNORECASE))

        # --- 1. PSYCHOLOGY LAWS ---
        # Hick's Law (heuristic; warn only)
        nav_items = len(re.findall(r'<NavLink|<Link|<a\s+href|nav-item', content, re.IGNORECASE))
        if nav_items > 7:
            self.warnings.append(f"[Hick's Law] {filename}: {nav_items} nav items (Recommended max 7)")
        ...
        for w in weights:
            ...
            if val is not None and val != "":
                ...
                try:
                    weight_values.append(int(str(val)))
                except: pass
