...
def check_page(file_path: Path) -> dict:
    ...
    # Detect if this is a layout/template file (actual <head> tag or Next.js <Head>)
    is_layout = bool(re.search(r'<head\b', content, re.IGNORECASE) or re.search(r'<Head\b', content))
    
    # 1. Title tag
    has_title = bool(re.search(r'<title\b', content, re.IGNORECASE))
